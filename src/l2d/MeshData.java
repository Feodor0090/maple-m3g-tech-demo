...
public class MeshData {
	...
	public static String getStringFromResource(String file) {
		...
		try {
			var1 = Object.class.getResourceAsStream(file);
	...
	public void setTexture(Image2D texture) {
		this.texture = texture;
		if (m3gMesh != null) m3gMesh.getAppearance(0).getTexture(0).setImage(texture); //todo why
	}
	...
	private static MeshData loadMesh3D2(
		...
		int[] zs = new int[submeshes.length];
		...
		for (int i = 0; i < submeshes.length; i++) {
			...
			int firstVertex = indices[indices.length - 1];
			short[] copy = new short[3];
			poses.get(firstVertex, 1, copy);
			zs[i] = copy[2];

			submeshes[i] = new TriangleStripArray(indices, stripLengths);
		}

		int[] layers = new int[submeshes.length];
		for (int i = 0; i < layers.length; i++) {
			float n = (zs[i]+7373) / 819f;
			layers[i] = (int) (n + 0.2f);
			ap[i].setLayer(layers[i]);
		}
	...
	private static MeshData loadMesh3D2_2(
			DataInputStream dis,
			Image2D img,
			float mdlScale,
			float scale,
		...
		int submeshesCount = dis.readUnsignedShort();

		Vector submeshesV = new Vector();
		...
		int[] counts = new int[0x0000ffff];
		short[] buf = new short[10];
		for (int i = 0; i < poses.getVertexCount(); i++) {
			poses.get(i, 1, buf);
			counts[buf[2] - Short.MIN_VALUE]++;
		}

		int[] layers = new int[0x0000ffff];
		...
		counts = null;
		short[] allPos = new short[poses.getVertexCount() * 3];
		poses.get(0, poses.getVertexCount(), allPos);
		...
		for (int i = 0; i < submeshesCount; i++) {
			...
			for (int p = 0; p < quads; p++) {
				...
				if (vtxCount <= 256) {
					li[0] = dis.readUnsignedByte();
					li[0 + 1] = dis.readUnsignedByte();
					li[0 + 2] = dis.readUnsignedByte();
					li[0 + 3] = dis.readUnsignedByte();
				} else {
					li[0] = dis.readUnsignedShort();
					li[0 + 1] = dis.readUnsignedShort();
					li[0 + 2] = dis.readUnsignedShort();
					li[0 + 3] = dis.readUnsignedShort();
				}
				...
				int z = allPos[li[0] * 3 + 2];
				app.setLayer(layers[z - Short.MIN_VALUE]);
				appV.addElement(app);
				submeshesV.addElement(new TriangleStripArray(li, sl));
			}
		...
		TriangleStripArray[] submeshes = new TriangleStripArray[submeshesV.size()];
		submeshesV.copyInto(submeshes);
		Appearance[] ap = new Appearance[submeshes.length];
		appV.copyInto(ap);
		/*for (int i = 0; i < submeshes.length; i++) {
			Appearance app = new Appearance();
		...
		return new MeshData(m3gMesh, aabbMin, aabbMax);
	}
